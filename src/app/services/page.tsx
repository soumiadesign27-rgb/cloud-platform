  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-16 md:px-6">
      <header className="mb-10 text-right">
        ...
        <p className="mt-2 text-slate">حلول جاهزة ينفذها فريقنا لمساعدتك على إطلاق وتطوير مشروعك.</p>
      </header>
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {services?.map((s) => (
          <div key={s.id} className="rounded-2xl border border-ink/10 bg-white p-6 transition hover:-translate-y-1 hover:shadow-lg">
            ...
            <div className="mt-5 flex items-center justify-between">
              ...
              <button className="rounded-full bg-ink px-4 py-2 text-xs font-bold text-parchment hover:bg-teal">
                اطلب الخدمة
              </button>
            </div>
          </div>
        ...
        {!services?.length && (
          <p className="col-span-full text-center text-slate">لا توجد خدمات متاحة حالياً.</p>
        )}
      </div>
    </div>
  );
